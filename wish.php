...
<!-- <div class="distance">
  ...
  <?php
        $s="select * from wishlist where user='$user_name' order by id desc";
        $res=mysqli_query($conn,$s);
...
<form action="place1.php?p1=<?php echo $row['place']?>&s1=<?php echo $cccc?>" method="post"><div class="column">
         <?php 
          ...
          <?php 
          $pp=$row['place'];
             $quew=mysqli_query($conn,"select * from wishlist where user='$user_name' and city='$cccc' and place='$pp'");
             $affectedw = mysqli_affected_rows($conn);
             if($affectedw == 0 && $quew){
          ?>
          <div class="top-left3"><input type="button" class="b1 log1" id="myBtn" value="   " onClick="Javascript:window.location.href = 'added1.php?place=<?php echo $row['place']?>&city=<?php echo $cccc?>';" />&nbsp;&nbsp;<div style="font-size: 22px;display: inline-block;">Add to Wishlist</div></div>
        <?php } 
        else
          { ?>
               <div class="top-left3"><input type="button" class="b2 log2" id="myBtn" value="   " onClick="Javascript:window.location.href = 'remo1.php?place=<?php echo $row['place']?>&city=<?php echo $cccc?>';" />&nbsp;&nbsp;<div style="font-size: 22px;display: inline-block;">Remove from Wishlist</div></div>
       <?php   }?>
